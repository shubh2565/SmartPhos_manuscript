...
```{r}
se1 <- se[, se$sampleType == "Phospho" & se$replicate != "rep1"]
...
g <- ggplot(plotTab, aes(x = sample, y = total)) +
  ...
  ggtitle("Total phosphosites for each sample") +
...
p$layers[[1]]$aes_params$stroke <- 1.0
...
plotVolcanoDEA(dea$resDE, fcFilter = 1, pFilter = 0.01, usePadj = FALSE, 
               title = "Phospho EGF 10 min vs Ctrl")
```
...
plotVolcanoDEA(dea20$resDE, fcFilter = 1, pFilter = 0.01, usePadj = FALSE,
               title = "Phospho EGF 20 min vs Ctrl")
```
```{r}
...
plotVolcanoDEA(dea40$resDE, fcFilter = 1, pFilter = 0.01, usePadj = FALSE, 
               title = "Phospho EGF 40 min vs Ctrl")
```
