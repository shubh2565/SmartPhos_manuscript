...
knitr::opts_chunk$set(echo = TRUE, dev = c("png","pdf"), fig.path = "./figureCommandLineEGF_FullProteome/")
...
plotIntensity(se[, se$sampleType == "FullProteome"], color = "replicate") + theme_classic() +
  theme(
...
```{r}
newSE <- preprocessProteome(seData = se, transform = "log2", 
...
p + coord_equal() + theme(aspect.ratio = 1, legend.position = "right",
                          axis.text = element_text(color = "#000000")) +
  geom_text_repel(aes(label = timepoint), max.overlaps = Inf, size = 2.75) +
  guides(
    shape = guide_legend(order = 1),
    color = guide_legend(order = 2)
    )
...
```
```{r volcanoPlot_notFromPackage_FullProt}
plotVolcanoDEA(dea$resDE, fcFilter = 1, pFilter = 0.01, usePadj = FALSE,
               title = "FullProteomics EGF 10 min vs Ctrl")
```
...
```{r intensityBoxPlot_specificPhophosite, eval=FALSE, include=FALSE}
...
newSEts <- addZeroTime(newSE, condition = "treatment", treat = "EGF",
                       zeroTreat = "Ctrl",
                       timeRange = c("10min", "20min","40min","100min", "3h", "6h"))
...
tsc <- clusterTS(x = exprMat, k = 5, pCut = 0.4)
```
...
```{r singleSiteTimeSeries, eval=FALSE, include=FALSE}
...
```{r kinaseActivity, eval=FALSE, include=FALSE}
...
```{r phosphosites, fig.path = "./figureCommandLineEGF_FullProteome/phosphosites/"}
newSEts <- addZeroTime(newSE, condition = "treatment", treat = "EGF",
                       zeroTreat = "Ctrl",
                       timeRange = c("10min", "20min","40min","100min"))
...
plotVolcanoDEA(dea20$resDE, fcFilter = 1, pFilter = 0.01, usePadj = FALSE,
               title = "FullProteomics EGF 20 min vs Ctrl")
```
```{r}
...
plotVolcanoDEA(dea40$resDE, fcFilter = 1, pFilter = 0.01, usePadj = FALSE,
               title = "FullProteomics EGF 40 min vs Ctrl")
```
...
plotVolcanoDEA(dea100$resDE, fcFilter = 1, pFilter = 0.01, usePadj = FALSE,
               title = "FullProteomics EGF 100 min vs Ctrl")
```

```{r}
dea3 <- performDifferentialExp(se = newSE, 
                              assay = "imputed", 
                              method = "limma", 
                              condition = "treatment", 
                              reference = "Ctrl", 
...
```{r volcanoPlot_fromPackage3h}
pFilter = 0.01
plotVolcano(dea3$resDE, pFilter = 0.01, fcFilter = 1) + theme_classic() +
  geom_hline(yintercept = -log10(as.numeric(pFilter)), color = "brown",
               linetype = "dashed") +
    annotate(x = 3.0, y = -log10(as.numeric(pFilter)) - 0.20,
             label = paste0("P-value = ", as.numeric(pFilter)),
...
```{r volcanoPlot_notFromPackage3h}
plotVolcanoDEA(dea3$resDE, fcFilter = 1, pFilter = 0.01, usePadj = FALSE,
               title = "FullProteomics EGF 3 h vs Ctrl")
```

```{r}
dea6 <- performDifferentialExp(se = newSE, 
                              assay = "imputed", 
                              method = "limma", 
                              condition = "treatment", 
                              reference = "Ctrl", 
...
```{r volcanoPlot_fromPackage6h}
pFilter = 0.01
plotVolcano(dea6$resDE, pFilter = 0.01, fcFilter = 1) + theme_classic() +
  geom_hline(yintercept = -log10(as.numeric(pFilter)), color = "brown",
               linetype = "dashed") +
    annotate(x = 3.0, y = -log10(as.numeric(pFilter)) - 0.20,
             label = paste0("P-value = ", as.numeric(pFilter)),
...
```{r volcanoPlot_notFromPackage6h}
plotVolcanoDEA(dea3$resDE, fcFilter = 1, pFilter = 0.01, usePadj = FALSE,
               title = "FullProteomics EGF 6 h vs Ctrl")
```
